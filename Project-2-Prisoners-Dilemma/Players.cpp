...
move
*/
Strategy Cooperator::strategy() const{
  return CooperatorStrategy;
}
Move Cooperator::move() {
...
move
*/
Strategy Cheater::strategy() const{
  return CheaterStrategy;
}
Move Cheater::move() {
  return CheatMove;
}
...
move
*/
Strategy Random::strategy() const{
...
void StatefulPlayer::beginRound(){
  ...
  _priorOpponenMove = NoMove;  
}
void StatefulPlayer::setOpponentMove(Move opponentMove){
  _priorOpponenMove = _opponentMove;
...
opponent just did. If you're into grudges, this is the right to decide
to hold one. After setting that member var, setOpponentMove must invoke
its superclass to perform the default behavior. 

*/
Strategy Grudger::strategy() const{
...
move
*/
Strategy Tit4Tat::strategy() const{
  return Tit4TatStrategy;
}
Move Tit4Tat::move(){
  // another way to say this
  return _opponentMove == NoMove ? CooperateMove : _opponentMove;
  // Move move = _opponentMove;
  // if(_opponentMove == NoMove)
  //   move = CooperateMove;
  // return move;
}
/*****************************************************************************/
...
move
*/
Strategy Tit4TatKitten::strategy() const{
  return Tit4TatKittenStrategy;
}
Move Tit4TatKitten::move(){
  bool cheat = _opponentMove == CheatMove && _priorOpponenMove == CheatMove;
  return cheat? CheatMove: CooperateMove;
}
/*****************************************************************************/
// Detective Methods
...
// opening sequence, they continue to play like Gruder players do, otherwise
// they play like tit4tat forever.
...
*/
Strategy Detective::strategy() const{
  return DetectiveStrategy;
}
...
Move Detective::move(){
  ...
  if(_openingIndex < _openingMovesCount){
    result = _openingMoves[_openingIndex++];
  } else if (_grudger){
    result = CheatMove;
  } else {
    result = _opponentMove;
  }
  return result;
}
void Detective::setOpponentMove(Move opponentMove){
  StatefulPlayer::setOpponentMove(opponentMove);
  if(_openingIndex < _openingMovesCount && _opponentMove == CheatMove){
    _grudger = true;
  }
}
